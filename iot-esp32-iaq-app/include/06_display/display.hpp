...
public:
    ...
    // Probes the OLED on the I2C bus; returns false if it does not respond. U8g2's HW I2C is
    // bound to the global `Wire`, so p_wire must wrap `Wire`; it's injected (rather than begun
    // here) so main owns the shared bus's lifecycle and clock.
