...
// the old BOOST_SML_CFG_DISABLE_MIN_SIZE remains honoured for backward compat.
#if defined(BOOST_SML_CFG_ENABLE_MIN_SIZE) && !defined(BOOST_SML_CFG_DISABLE_MIN_SIZE)
#define BOOST_SML_DETAIL_ZERO_SIZE_ARRAY(...)                                                                          \
    __VA_ARGS__ _[0] {}
#else
#define BOOST_SML_DETAIL_ZERO_SIZE_ARRAY(...) static_assert(true, "")
#endif
...
static_assert(aux::is_same<collapse_const_refs_t<type_list<const none_type&>>, type_list<const none_type&>>::value,
              "`const U &` should collapse to `const U &` if only const refs are present");
static_assert(
    aux::is_same<collapse_const_refs_t<type_list<const none_type&, none_type&>>, type_list<none_type&>>::value,
    "`const U &` should collapse to `U &` if at least one non-const ref is present");
...
template<class T>
constexpr missing_ctor_parameter<T> try_get(...); // fallback: not in pool
template<class T>
constexpr T try_get(const pool_type<T>*); // #1 value
template<class T>
constexpr const T& try_get(const pool_type<const T&>*); // #3 const-ref
template<class T>
constexpr T& try_get(const pool_type<T&>*); // #2 mutable-ref
template<class T>
constexpr const T* try_get(const pool_type<const T*>*); // #5 const-ptr
template<class T>
constexpr T* try_get(const pool_type<T*>*); // #4 ptr
...
template<class T,
         class D,
         BOOST_SML_DETAIL_REQUIRES(!aux::is_same<aux::remove_const_t<T>, D>::value && aux::is_base_of<T, D>::value)>
...
struct pool_type_impl<T&,
    ...
    : pool_type_base {
    constexpr explicit pool_type_impl(T& val)
        : value{val} {} // bind ref directly
    template<class TObject>
    constexpr explicit pool_type_impl(TObject val)
        : value_{val}
        , value{value_} {} // default-construct backing store
...
struct missing_ctor_parameter {
    ...
    template<class U,
             BOOST_SML_DETAIL_REQUIRES(!aux::is_base_of<pool_type_base, U>::value && aux::is_constructible<U>::value)>
    ...
    template<class TMissing,
             BOOST_SML_DETAIL_REQUIRES(!aux::is_base_of<pool_type_base, TMissing>::value &&
                                       !aux::is_constructible<TMissing>::value)>
...
template<
    class T,
    class D,
    typename aux::enable_if<!aux::is_same<aux::remove_const_t<T>, D>::value && aux::is_base_of<T, D>::value, int>::type>
...
struct pool : pool_type<Ts>... {
    ...
    constexpr explicit pool(Ts... ts)
        : pool_type<Ts>(ts)... {} // direct construction from dep values
...
struct zero_wrapper<R (TBase::*)(TArgs...), T> {
    ...
    constexpr explicit zero_wrapper(type fn_ptr)
        : ptr{fn_ptr} {}
...
struct zero_wrapper<R (*)(TArgs...), T> {
    using type = R (*)(TArgs...);
    explicit zero_wrapper(type fn_ptr)
        : ptr{fn_ptr} {}
...
// are length-independent.
    ...
    return detail::get_type_name<T, 71>(__PRETTY_FUNCTION__,
                                        make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 71 - 2>{});
#endif
}
...
struct action_wrap {
    constexpr explicit action_wrap(F f)
        : f_(f) {}
...
public:
    ...
    constexpr explicit queue_event_call(const call_t& call_fn)
        : call{call_fn} {}
    call_t call{};
};
...
struct exception : internal_event {
    ...
    constexpr explicit exception(const TException& ex = {})
        : exception_(ex) {}
...
struct get_mapping_event_same_impl {
    ...
    using merged_inherit = aux::conditional_t<exists, extend_mapping_t<NewSM, RSMs...>, aux::inherit<RSMs..., NewSM>>;
    using type = aux::type_list<event_mappings<E, merged_inherit>>;
};
template<class E, class NewSM, class... RSMs>
struct get_mapping<event<E>, event<E>, event_mappings<E, aux::inherit<NewSM>>, event_mappings<E, aux::inherit<RSMs...>>>
...
struct get_state_mapping<sm<T>, TMappings, TUnexpected> {
    ...
    struct type : sub_sm_mapping {
        ...
        constexpr static bool
        execute(const TEvent& event, TSm& sm, TDeps& deps, TSubs& subs, typename TSm::state_t& current_state) {
            const auto sub_handled =
                sub_sm_mapping::template execute<TEvent, TSm, TDeps, TSubs>(event, sm, deps, subs, current_state);
            if (sub_handled && !sub_sm<sm_impl<T>>::get(&subs).is_terminated()) {
                return true;
            }
            return fallback_event_mapping::template execute<TEvent, TSm, TDeps, TSubs>(
...
get_event_mapping_impl(event_mappings<T1, aux::inherit<T1Mappings...>>*,
...
struct fold_expr {
    ...
    constexpr static bool dispatch(sm_impl& self,
        ...
        return dispatch_impl<TMappings>(
            self, current_state, aux::make_index_sequence<sizeof...(TStates)>{}, event, deps, subs, states);
    }
};
#endif
} // namespace policies
...
struct sm_impl
    ...
    using events_t =
        aux::apply_t<aux::unique_t,
                     aux::join_t<sub_internal_events_t,
                                 ...
                                 aux::apply_t<get_process_events, aux::apply_t<merge_deps, transitions_t>>>>;
    ...
    constexpr bool process_event(const TEvent& event, TDeps& d, TSubs& subs) {
        ...
        do {
            ...
            process_queued_events(d, subs, queued_handled, aux::type_wrapper<process_queue_t<TEvent>>{}, events_t{}));
...
#if BOOST_SML_DISABLE_EXCEPTIONS
        return process_event_impl<get_event_mapping_t<get_generic_t<TEvent>, mappings>>(
            event, d, subs, states_t{}, current_state);
#else
        return process_event_except_impl<get_event_mapping_t<get_generic_t<TEvent>, mappings>>(
            event, d, subs, current_state, has_exceptions{});
#endif
    }
    template<class TEvent,
             class TDeps,
    ...
    constexpr bool process_event_impl(
        const TEvent& event, TDeps& d, TSubs& subs, const aux::type_list<TStates...>& states, aux::index_sequence<0>) {
    ...
    template<class TMappings, class TEvent, class TDeps, class TSubs, class... TStates>
    constexpr bool process_event_impl(
        const TEvent& event, TDeps& d, TSubs& subs, const aux::type_list<TStates...>& states, state_t& current_state) {
    ...
    template<class TMappings, class TEvent, class TDeps, class TSubs>
    constexpr bool
    process_event_except_impl(const TEvent& event, TDeps& d, TSubs& subs, state_t& current_state, aux::false_type) {
        return process_event_impl<TMappings>(event, d, subs, states_t{}, current_state);
    }
    template<class TMappings, class TEvent, class TDeps, class TSubs>
    bool process_event_except_impl(
        const TEvent& event, TDeps& d, TSubs& subs, state_t& current_state, aux::true_type) noexcept {
        try {
            return process_event_impl<TMappings>(event, d, subs, states_t{}, current_state);
        } catch (...) {
    ...
    constexpr bool process_defer_events(
        TDeps&, TSubs&, const bool, const aux::type_wrapper<no_policy>&, const aux::type_list<TEvents...>&) {
        return false;
    }
    ...
    bool process_defer_events(TDeps& d,
                              TSubs& subs,
                              const bool handled,
    ...
    constexpr bool process_queued_events(
        TDeps&, TSubs&, bool&, const aux::type_wrapper<no_policy>&, const aux::type_list<TEvents...>&) {
        return false;
    }
    template<class TDeps, class TSubs, class TEvent>
    ...
    template<class TDeps, class TSubs, class TDeferQueue, class... TEvents>
    bool process_queued_events(TDeps& d,
                               TSubs& subs,
                               bool& queued_handled,
                               const aux::type_wrapper<TDeferQueue>&,
                               const aux::type_list<TEvents...>&) {
    ...
    constexpr bool is_terminated_impl(aux::index_sequence<Ns...>) const {
        ...
        auto result = true;
        (void)aux::swallow{
            0, (result &= current_state_[Ns] == aux::get_id_safe<state_t, terminate_state>((states_ids_t*)0))...};
        return result;
#endif
    }
    ...
    constexpr void clear_deferred_events() {
        clear_deferred_impl_(aux::integral_constant<bool, !aux::is_same<defer_t, no_policy>::value>{});
    }
...
private:
    ...
    using deps_t = aux::apply_t<
        aux::pool,
        aux::collapse_const_refs_t<aux::apply_t<
            aux::unique_t,
            aux::join_t<explicit_dep_list, dep_list, sm_all_t, logger_dep_t, aux::apply_t<merge_deps, sub_sms_t>>>>>;
...
public:
    ...
    constexpr void flush_queue() { aux::get<sm_impl<Tsm>>(sub_sms_).flush_queue(deps_, sub_sms_); }
    template<class T = aux::identity<sm_t>,
             class TVisitor,
             BOOST_SML_DETAIL_REQUIRES(concepts::callable<void, TVisitor>::value)>
    ...
    template<class T = aux::identity<sm_t>,
             class... TStates,
             BOOST_SML_DETAIL_REQUIRES(sizeof...(TStates) ==
    ...
    template<class T = aux::identity<sm_t>,
             class... TStates,
             BOOST_SML_DETAIL_REQUIRES(!aux::is_same<no_policy, typename Tsm::testing_policy>::value &&
                                       aux::always<T>::value)>
...
template<class T, class E>
auto args_(int)
    -> aux::function_traits_t<decltype(&T::BOOST_SML_DETAIL_TEMPLATE_KEYWORD operator()<back::get_event_t<E>>)>;
template<class T, class E>
...
constexpr decltype(auto)
get_arg(const aux::type_wrapper<back::process<TEvents...>>&, const TEvent&, Tsm&, TDeps&, TSubs& subs, int) {
...
template<class T,
         ...
         class Tsm,
         class TDeps,
         BOOST_SML_DETAIL_REQUIRES(
             !aux::is_same<aux::remove_const_t<aux::remove_reference_t<back::get_event_t<TEvent>>>, T>::value &&
             aux::is_base_of<aux::pool_type<T&>, TDeps>::value)>
...
constexpr auto get_arg(const aux::type_wrapper<T>&, const TEvent&, Tsm&, TDeps&, TSubs& subs, int)
...
constexpr decltype(auto)
get_arg(const aux::type_wrapper<T>& tw, const TEvent& event, Tsm& sm, TDeps& deps, TSubs&, ...) {
...
struct call<TEvent, aux::type_list<Ts...>, TLogger> {
    ...
    execute_impl(const aux::type_wrapper<bool>&, T object, const TEvent& event, SM& sm, TDeps& deps, TSubs& subs) {
    ...
    template<class Tsm, class T, class SM, class TDeps, class TSubs>
    constexpr static auto
    execute_impl(const aux::type_wrapper<void>&, T object, const TEvent& event, SM& sm, TDeps& deps, TSubs& subs) {
        back::policies::log_action<Tsm>(aux::type_wrapper<TLogger>{}, deps, object, event);
...
template<class T>
struct is_integral_constant_like<T, aux::void_t<decltype(T::value), typename T::type>>
    : aux::integral_constant<bool,
                             aux::is_same<bool, aux::remove_const_t<decltype(T::value)>>::value &&
                                 aux::is_same<T, typename T::type>::value> {};
template<class T, BOOST_SML_DETAIL_REQUIRES(concepts::callable<bool, T>::value && !is_integral_constant_like<T>::value)>
constexpr auto operator!(const T& t) {
    return front::not_<aux::zero_wrapper<T>>(aux::zero_wrapper<T>{t});
}
template<class T1,
         class T2,
         BOOST_SML_DETAIL_REQUIRES(concepts::callable<bool, T1>::value&& concepts::callable<bool, T2>::value &&
                                   !is_integral_constant_like<T1>::value && !is_integral_constant_like<T2>::value)>
...
template<class T1,
         class T2,
         BOOST_SML_DETAIL_REQUIRES(concepts::callable<void, T1>::value&& concepts::callable<void, T2>::value)>
constexpr auto operator,(const T1& t1, const T2& t2) {
...
struct process {
    ...
    public:
        constexpr explicit process_impl(const TEvent& ev)
            : event(ev) {}
        template<class T, class Tsm, class TDeps, class TSubs>
...
struct transition<state<S2>, G, A> : transition<state<internal>, state<S2>, front::event<back::anonymous>, G, A> {
    ...
    constexpr transition(const G& g_init, const A& a_init)
        : transition<state<internal>, state<S2>, front::event<back::anonymous>, G, A>{g_init, a_init} {}
    template<class T>
...
constexpr void update_composite_states(TSubs& subs, aux::false_type, Ts&&...) {
    ...
    aux::for_each(typename T::initial_states_t{}, [&](auto state_identity) {
        ...
        recurse_composite<state_t>(subs, aux::integral_constant<bool, detail::is_composite_state_v<state_t>>{});
    });
}
...
constexpr void update_current_state(SM&,
    ...
    back::policies::log_state_change<typename SM::sm_t>(
        aux::type_wrapper<typename SM::logger_t>{}, deps, aux::string<typename TSrcState::type>{}, aux::string<T>{});
    current_state = new_state;
...
struct transition<state<S1>, state<S2>, front::event<E>, G, A> {
    ...
    template<class TEvent, class SM, class TDeps, class TSubs>
    constexpr bool
    execute(const TEvent& ev, SM& sm, TDeps& d, TSubs& subs, typename SM::state_t& current_state, aux::true_type) {
    ...
    template<class TEvent, class SM, class TDeps, class TSubs>
    constexpr bool
    execute(const TEvent& ev, SM& sm, TDeps& d, TSubs& subs, typename SM::state_t& current_state, aux::false_type) {
        if (call<TEvent, args_t<G, TEvent>, typename SM::logger_t>::execute(g, ev, sm, d, subs)) {
            update_current_state(sm,
                                 d,
                                 subs,
...
struct transition<state<S1>, state<S2>, front::event<E>, always, A> {
    ...
    constexpr transition(always, const A& a_init)
        : a(a_init) {}
    template<class TEvent, class SM, class TDeps, class TSubs>
    constexpr bool
    execute(const TEvent& ev, SM& sm, TDeps& d, TSubs& subs, typename SM::state_t& current_state, aux::true_type) {
...
struct transition<state<S1>, state<S2>, front::event<E>, G, none> {
    ...
    constexpr transition(const G& g_init, const none&)
        : g(g_init) {}
    template<class TEvent, class SM, class TDeps, class TSubs>
    constexpr bool
    execute(const TEvent& ev, SM& sm, TDeps& d, TSubs& subs, typename SM::state_t& current_state, aux::true_type) {
...
struct transition<state<S1>, state<S2>, front::event<E>, always, none> {
    ...
    template<class TEvent, class SM, class TDeps, class TSubs>
    constexpr bool
    execute(const TEvent&, SM& sm, TDeps& d, TSubs& subs, typename SM::state_t& current_state, aux::false_type) {
        update_current_state(sm,
                             d,
                             subs,
                             current_state,
...
template<class... Ts,
         BOOST_SML_DETAIL_REQUIRES(aux::is_same<aux::bool_list<aux::always<Ts>::value...>,
